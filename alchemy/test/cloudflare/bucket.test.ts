...
describe("R2 Bucket Resource", async () => {
	...
	test("bucket with custom domain", async (scope) => {
		...
		if (!ZONE_NAME) {
			console.log("Skipping custom domain test - ALCHEMY_TEST_DOMAIN not set");
			return;
		}

		const customDomain = `r2-test.${ZONE_NAME}`;

		// Get the zone from the domain name
		const zone = await getZoneByDomain(api, ZONE_NAME);
		if (!zone) {
			console.log(
				`Skipping custom domain test - zone not found for ${ZONE_NAME}`,
			);
			return;
		}
		...
		try {
			...
			expect(bucket.customDomain?.zoneId).toEqual(zone.id);
			...
			const dnsResponse = await api.get(
				`/zones/${zone.id}/dns_records/${bucket.customDomain!.dnsRecordId}`,
			);
			...
			const updatedDomain = updatedDomains.find(
				(d) => d.domain === customDomain,
			);
			...
			const deletedDnsResponse = await api.get(
				`/zones/${zone.id}/dns_records?name=${customDomain}`,
			);
